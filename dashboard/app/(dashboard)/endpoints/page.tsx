...
function EndpointForm({
  ...
  const inputClass =
    "w-full px-3 py-2.5 bg-elevated border border-border rounded-lg text-sm text-text-primary placeholder:text-text-muted";
...
export default function EndpointsPage() {
  const {
    ...
    isLoading,
    mutate,
  } = useSWR<Endpoint[]>("/endpoints", fetcher);
  ...
  const handleToggleStatus = async (ep: Endpoint) => {
    ...
    try {
      ...
      await mutate();
      toast.success(
        `Endpoint ${newStatus === "PAUSED" ? "paused" : "resumed"}`
      );
    } catch (err) {
      toast.error(
  ...
  const isEmpty = !isLoading && (!endpoints || endpoints.length === 0);
                ...
                <tr key={ep.id}>
                  {editingId === ep.id ? (
                    ...
                    <>
                      ...
                      <td className="px-4 py-3 text-text-secondary font-mono text-sm truncate max-w-xs">
                        {ep.url}
                      </td>
